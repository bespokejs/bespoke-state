...
var state = require('../../lib-instrumented/bespoke-state.js');
...
describe("bespoke-state", function() {
  ...
  describe("when a slide without a 'data-bespoke-state' attribute is activated", function() {
    ...
    beforeEach(function() {
      createDeck();
      deck.parent.classList.add('existing-class1');
      deck.parent.classList.add('existing-class2');
      deck.slide(1);
    });

    it("should leave the parent classes untouched", function() {
      expect(deck.parent.classList.contains('existing-class1')).toBe(true);
      expect(deck.parent.classList.contains('existing-class2')).toBe(true);
    });
